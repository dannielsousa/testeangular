...
export class CadastroRealizadoComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < this.respostas.length; i++) {
      this.pergResp.pergunta = this.perguntas[i];
      this.pergResp.resposta = this.respostas[i];
      this.perguntasRespostas.push(this.pergResp);
    }

    console.log(this.perguntasRespostas);
